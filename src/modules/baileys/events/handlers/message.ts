import { BaileysEventEmitter, WAMessage, WAMessageKey, jidNormalizedUser, toNumber } from "@whiskeysockets/baileys";
    ...
    const upsert: BaileysEventHandler<'messages.upsert'> = async ({ messages, type }) => {
        ...
        for (const message of messages) {
            ...
            await messageCollection.updateOne(
                { sessionId_remoteJid_id: { remoteJid: jid, id: message.key.id, sessionId } },
                { $set: { ...data, remoteJid: jid, id: message.key.id, sessionId } },
                { upsert: true }
            );

            const chatExists = await chatCollection.countDocuments({ id: jid, sessionId });
    ...
    const updateReceipt: BaileysEventHandler<'message-receipt.update'> = async (updates) =>{
        for (const { key, receipt } of updates) {
            try {
                    ...
                    if (!message) {
                        logger.debug({ update }, 'Got receipt update for non existent message');
                        return;
                    }
    
                    const userReceipts = message.userReceipt || [];
                    const existingReceiptIndex = userReceipts.findIndex((r:any) => r.userJid === receipt.userJid);
    ...
    const updateReaction: BaileysEventHandler<'messages.reaction'> = async (reactions) => {
        for (const { key, reaction } of reactions) {
            try {
                    ...
                    if (!message) {
                        logger.debug({ update }, 'Got reaction update for non existent message');
                        return;
                    }

                    const authorID = getKeyAuthor(reaction.key);
                    const existingReactions = message.reactions || [];
                    const filteredReactions = existingReactions.filter((r:any) => getKeyAuthor(r.key) !== authorID);
